...
/*
 * When a DR scan arrives without a prior IR scan to pair with, we send a
 * zero-valued placeholder IR scan of this length so the batch is still
 * a valid combined IR+DR entry on the wire.
 */
...
static int cklink_execute_scan(struct jtag_command *cmd)
{
	...
	const uint8_t *ir_value = ck->ir_bits ? ck->ir_value : NULL;
...
{
	const uint8_t tms_pattern = CKLINK_TAP_RESET_TMS_PATTERN;
	return cklink_jtag_scan(cklink_handle,
				CKLINK_TAP_RESET_BITS, &tms_pattern,
				0, NULL, NULL);
}
